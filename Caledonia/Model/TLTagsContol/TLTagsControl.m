...
- (id)initWithFrame:(CGRect)frame andTags:(NSArray *)tags withTagsControlMode:(TLTagsControlMode)mode {
    ...
    if (self != nil) {
        ...
        [self setbackgroundColor:[[NSMutableArray alloc]initWithArray:_tagsbackgroundColor]];
        [self setMode:mode];
    }
...
- (void)commonInit {
    ...
    _tagsbackgroundColor = [NSMutableArray array];
...
- (void)reloadTagSubviews {
    ...
    for (int i = 0; i <self.tags.count; i ++) {
        ...
        tagView.layer.cornerRadius = tagView.layer.frame.size.height/2;
...
- (void)setbackgroundColor:(NSMutableArray *)color {
    _tagsbackgroundColor = color;
}
